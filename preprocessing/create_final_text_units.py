...
def build_steps(
    ...
    * `workflow:create_final_communities`
    ...
    text_unit_text_embed_config = {'strategy': {'type': "openai", 'llm': {'api_key': '<API_KEY>', 'type': "openai_embedding", 'model': 'nomic_embed_text', 'max_tokens': 4000, 'temperature': 0, 'top_p': 1, 'request_timeout': 180.0, 'api_base': 'http://localhost:11434/api', 'api_version': None, 'organization': None, 'proxy': None, 'cognitive_services_endpoint': None, 'deployment_name': None, 'model_supports_json': None, 'tokens_per_minute': 0, 'requests_per_minute': 0, 'max_retries': 10, 'max_retry_wait': 10.0, 'sleep_on_rate_limit_recommendation': True, 'concurrent_requests': 25}, 'stagger': 0.3, 'num_threads': 50, 'batch_size': 16, 'batch_max_tokens': 8191}} # config.get("text_unit_text_embed", base_text_embed)
    covariates_enabled = False # config.get("covariates_enabled", False)
    skip_text_unit_embedding = True # config.get("skip_text_unit_embedding", False)
